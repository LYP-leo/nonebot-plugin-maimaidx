...
async def get_chartInfo_b50(qqid: int, username: Optional[str], mode, mode_str):
    ...
    if maiconfig.maimaidxtoken:
        _obj = await maiApi.query_user_dev(qqid=qqid, username=username)
        data = [PlayInfoDev(**_d) for _d in _obj['records']]
    else:
        version = list(set(_v for _v in list(plate_to_version.values())))
        _obj = await maiApi.query_user('plate', qqid=qqid, username=username, version=version)
        for _d in _obj['verlist']:
    ...
    for _d in data:
        ...
        if mode == 'fit':
            if music.stats != None:
                ds = round(music.stats[_d.level_index].fit_diff, 2)
        else:
            ds = music.ds[_d.level_index]
        a: float = _d.achievements
        ra, rate = computeRa(ds, a, israte=True)

        version_name = music.basic_info.version
        type_str = music.type
        ...
        chart: Dict[str] = {
            ...
            'level_label': diffs[_d.level_index],
            'ra': ra,
            'rate': rate,
            'song_id': _d.song_id,
            'title': _d.title,
...
async def get_songs_by_ach(qqid: int, username: Optional[str], obj, mode, mode_str):
    # rating 先清零
    obj['rating'] = 0
    charts = await get_chartInfo_b50(qqid, username, mode, mode_str)
    obj['charts'] = charts
...
async def generate(qqid: Optional[int] = None, username: Optional[str] = None, mode: str = None, mode_str: list[str] = None) -> str:
    try:
        ...
        if mode:
            await get_songs_by_ach(qqid, username, obj, mode, mode_str)
