...
async def get_songs_by_ach(mode, mode_str, obj, qqid: int, username: Optional[str]):
    ...
    sd: Optional[List[Dict[str]]] = []
    dx: Optional[List[Dict[str]]] = []
    ...
    for _d in data['verlist']:
        ...
        chart: Dict[str] = {
            ...
            'ds': ds,
            'dxScore': 0,  # 没有开发者Token, 无法获得歌曲dxScore
            'fc': _d['fc'],
            'fs': _d['fs'],
            'level': _d['level'],
    ...
    sd.sort(key=lambda _data: _data['ra'], reverse=True)        # 按ra排序
    sd = sd[0:35]                                               # 取前35个

    dx.sort(key=lambda _data: _data['ra'], reverse=True)        # 按ra排序
    dx = dx[0:15]                                               # 取前15个
...
async def generate(qqid: Optional[int] = None, username: Optional[str] = None, mode: str = None, mode_str: list[str] = None) -> str:
    try:
        ...
        # 查看obj当中各个元素的类型
        # print('type of obj[\'charts\']:', type(obj['charts']))                    # <class 'dict'>
        # print('type of obj[\'charts\'][\'dx\']:', type(obj['charts']['dx']))      # <class 'list'>
        # print('type of obj[\'charts\'][\'sd\']:', type(obj['charts']['sd']))      # <class 'list'>
